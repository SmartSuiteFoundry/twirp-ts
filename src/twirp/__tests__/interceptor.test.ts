import { chainInterceptors, Interceptor } from "../interceptors.js";
...
describe("Interceptor", () => {
  it("will chain interceptors", async () => {
    const spy = jest.fn();
    const interceptor0: Interceptor<TwirpContext, any, any> = async (
      ctx,
      typedRequest,
      next
    ) => {
    ...
    const spy1 = jest.fn();
    const interceptor1: Interceptor<TwirpContext, any, any> = async (
      ctx,
      typedRequest,
      next
    ) => {
    ...
    const chain = chainInterceptors(interceptor0, interceptor1) as Interceptor<
      TwirpContext,
      any,
      any
    >;
